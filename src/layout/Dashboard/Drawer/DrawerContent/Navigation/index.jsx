...
export default function Navigation() {
  ...
  // If the number of items exceeds HORIZONTAL_MAX_ITEM, prepare the remaining items
  if (lastItem && lastItem < menuItems.items.length) {
    lastItemId = menuItems.items[lastItem - 1].id;
    lastItemIndex = lastItem - 1;
  ...
  const navGroups = menuItems.items.slice(0, lastItemIndex + 1).map((item, index) => {
    switch (item.type) {
      case 'group':
        if (item.url && item.id !== lastItemId) {
          return (
            <List key={item.id} sx={isHorizontal ? { mt: 0.5 } : {}}>
              {!isHorizontal && index !== 0 && <Divider sx={{ my: 0.5 }} />}
              <NavItem item={item} level={1} isParents setSelectedID={setSelectedID} />
            </List>
          );
  ...
  return (
    <Box
      sx={{
        pt: isHorizontal ? 0 : 2,
        '& > ul:first-of-type': !isHorizontal ? { mt: 0 } : {},
        display: isHorizontal ? { xs: 'block', lg: 'flex' } : 'block'
      }}
    >
      {navGroups}
